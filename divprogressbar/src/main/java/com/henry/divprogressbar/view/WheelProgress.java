package com.henry.divprogressbar.view;
...
import com.henry.divprogressbar.R;
...
public class WheelProgress extends View {
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.WheelProgress_barWidth,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.WheelProgress_rimWidth,
                rimWidth);

        spinSpeed = (int) a.getDimension(R.styleable.WheelProgress_spinSpeed,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.WheelProgress_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.WheelProgress_barColor, barColor);

        barLength = (int) a.getDimension(R.styleable.WheelProgress_barLength,
                barLength);

        textSize = (int) a.getDimension(R.styleable.WheelProgress_textSize,
                textSize);

        textColor = (int) a.getColor(R.styleable.WheelProgress_textColor,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.WheelProgress_text)) {
            setText(a.getString(R.styleable.WheelProgress_text));
        }

        rimColor = (int) a.getColor(R.styleable.WheelProgress_rimColor,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.WheelProgress_circleColor,
                circleColor);

        contourColor = a.getColor(R.styleable.WheelProgress_contourColor, contourColor);
        contourSize = a.getDimension(R.styleable.WheelProgress_contourSize, contourSize);
