 ...
 * @author holiy
 */
public class BaseSidingMenuV5 extends HorizontalScrollView {
     ...
     * @param defStyle
     */
    public BaseSidingMenuV5(Context context, AttributeSet attrs, int defStyle) {
...
//		mMenuRightPadding = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, context.getResources().getDisplayMetrics());
    ...
    public BaseSidingMenuV5(Context context) {
        this(context, null, -1);
    }
     ...
     * @param attrs
     */
    public BaseSidingMenuV5(Context context, AttributeSet attrs) {
    ...
    private void mSmoothScrollTo(final int x, final int y){
        new Handler().post(new Runnable() {
            @Override
            public void run() {
                BaseSidingMenuV5.this.smoothScrollTo(x, y);//隐藏菜单，显示内容
            }
        });
    }
...
//		this.smoothScrollTo(2 * mMenuWidth, 0);
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if(l <= mMenuWidth){//内容区和左滑菜单
            ...
            ViewHelper.setTranslationX(mLeftMenu, mMenuWidth*scale*0.7f);//右边参数是初始隐藏大小
            //------------------菜单的效果-----------------
            ...
            //缩放比例
            float leftScale = 0.7f + 0.3f*rScale;
            float rightScale = 1.0f - rScale*0.4f;
            //透明度
            float rightAlpha = 0.5f + 0.5f*(1-rScale);
            ViewHelper.setTranslationX(mRightMenu, mMenuWidth*rScale*0.7f);//右边参数是初始隐藏大小
